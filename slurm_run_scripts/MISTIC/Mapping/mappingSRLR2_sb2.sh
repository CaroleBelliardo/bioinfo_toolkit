...
#SBATCH --cpus-per-task=240     # number of CPU per task #4
#SBATCH --nodes=1   # number of nodes
#SBATCH --mem=1200G   # memory per Nodes   #38
...
$SING2 $SING_IMG sambamba sort --sort-by-name -m 1200G -t $SLURM_JOB_CPUS_PER_NODE -o $bam.sorted --tmpdir ./tmp_sambamba_$5 $bam
check_command

$SING2 $SING_IMG sambamba index --threads $SLURM_JOB_CPUS_PER_NODE --tmpdir ./tmp_sambamba_$5 $bam.sorted
check_command

$SING2 $SING_IMG sambamba flagstat -p -m 1200G -t $SLURM_JOB_CPUS_PER_NODE --tmpdir ./tmp_sambamba_$5 $bam.sorted > $bam.sorted_flasgstat


$SING2 $SING_IMG sambamba markdup -r -p -m 1200G --nthreads $SLURM_JOB_CPUS_PER_NODE --tmpdir ./tmp_sambamba_$5 -o $bam.sorted.rmdup $bam.sorted
check_command

$SING2 $SING_IMG sambamba flagstat -p -m 1200G -t $SLURM_JOB_CPUS_PER_NODE $bam.sorted.rmdup --tmpdir ./tmp_sambamba_$5 > $bam.sorted.rmdup_flasgstat
