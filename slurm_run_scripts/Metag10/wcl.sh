...
fileOutput=/lerins/hub/projects/25_IPN_Metag/10Metag/len/${PREF}.len

cat $FILENAME | awk '$0 ~ ">" {print c; c=0;printf substr($0,2,100) "\t"; } $0 !~ ">" {c+=length($0);} END { print c; }' > $fileOutput
