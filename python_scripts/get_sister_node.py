...
"""
  Usage:
    get_sister_nodes2.py -g <FILE> -t <REPO>

  Options:
    -g, --gene_list <FILE>   gene list
    -t, --treeRepo <REPO>    tree repositoy
"""
...
def main():
    ...
    tree_Repo = args['--treeRepo']

    synthTab=pandas.DataFrame()
    with open(gene_list, 'r') as genetab :
        for line in genetab:
            l = line.strip().split('\t')
            if len (l) < 3 : continue
            gene = l[1]
            groupeNb=l[3].strip('.fa')
            phylogeny_file = tree_Repo + "/" + l[3] + '.fasttree'
            node_support = 0
            result_tree = analyze_tree(phylogeny_file,gene+"@StudiedOrganism", node_support)
            tab=run_node_analyse(result_tree,gene,groupeNb)
            synthTab = pandas.concat([synthTab, tab])

    synthTab.to_csv('test.Txt', sep='\t')
